...
CODE,
        ...
        [
            ...
            'paragraphs' => [
                ...
                'Общие предметные правила контактов и клиентов разделяются с HTML-интерфейсом и импортом через ContactWriteService и ClientWriteService. Ресурсный API-сервис проверяет только структуру JSON и разрешает внешние имена в id; обязательные поля, формат email, внутреннюю почту и дубликаты проверяет write-сервис. Новое правило, действующее во всех каналах, следует добавлять на эту общую границу.',
            ],
            'examples' => [
                [
...
CODE,
        ...
        [
            ...
            'paragraphs' => [
                'Любой POST создания обрабатывается через AbstractApiService::batch(). Для каждого элемента Database::transaction() открывает транзакцию на общем подключении Illuminate Database, после чего ресурсный сервис вызывает общий write-сервис. Вложенный write-сервис видит открытую транзакцию и присоединяется к ней. ApiException, PDOException или другая ошибка откатывает только текущий элемент; обработка следующих элементов продолжается.',
