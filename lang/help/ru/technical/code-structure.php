...
public_html/index.php                    # require_once, объект, маршруты',
    ...
    array (
      ...
      array (
        ...
        1 => 'В проекте нет моделей Eloquent и автоматического runner для миграций: доступ к данным выполняется через Illuminate Database Query Builder, а изменения существующей базы применяются вручную SQL-файлами из database/migrations. database/schema.sql предназначен для первой установки и не является миграцией. Каталога автоматических тестов пока нет, поэтому изменение проверяется PHP lint, сценариями интерфейса и API и просмотром журналов; критичные предметные правила желательно постепенно покрывать модульными или интеграционными тестами.',
