          ...
          'code' => 'public function paginate(int $page, int $perPage, string $sort): array
{
    ...
    array (
      ...
      array (
        ...
        1 => 'Сервис может использовать несколько репозиториев и другие сервисы, но не должен зависеть от HTML. Если операция меняет несколько связанных наборов данных, сервис открывает транзакцию, делает commit только после всех шагов и обязательно выполняет rollBack при исключении.',
...
<h1><?= t(\'projects.title\') ?></h1>',
    ...
    array (
      ...
      'paragraphs' => 
      array (
        0 => 'API использует тот же Front Controller и Router, но имеет отдельную цепочку классов. Конкретный контроллер сообщает имя ресурса и создаёт сервис. AbstractApiController реализует стандартные CRUD-методы, аутентификацию, scopes, разбор JSON, единый формат ошибок, X-Request-Id и запись api_logs. Это применение Template Method: общий протокол находится в базовом классе, различия ресурса — в наследнике и сервисе.',
        ...
        2 => 'Пакетное создание обрабатывается AbstractApiService::batch(): каждый элемент получает отдельную транзакцию и отдельный результат, а общий ответ имеет статус 207. При добавлении API-ресурса следует расширять эти базовые классы, а не копировать обработку ключей, JSON и журналирования.',
      ),
      'examples' => 
      array (
        0 => 
        array (
          ...
          'code' => 'class ProjectApiController extends AbstractApiController
{
...
throw new ApiException(
    ...
    array (
      ...
      'paragraphs' => 
      array (
        0 => 'Например, создание контакта через интерфейс начинается в POST-маршруте ContactController. Контроллер проверяет contacts.create и CSRF уже проверен общей точкой входа. Затем он нормализует поля, проверяет обязательные значения, классифицирует email, вызывает ContactRepository и синхронизирует связи, теги и пользовательские поля. После успеха выполняется redirect на страницу контактов.',
        ...
        2 => 'Таким образом, HTML-интерфейс и API имеют разные транспортные контроллеры, но используют общие репозитории и предметные сервисы. Если правило должно действовать в обоих каналах, его нельзя оставлять только в JavaScript или одном контроллере — его следует вынести в общий сервис либо доменный helper.',
      ),
      'examples' => 
      array (
        0 => 
