          ...
          'code' => 'public function store(): void
{
    ...
    array (
      ...
      'paragraphs' => 
      array (
        0 => 'Repository изолирует доступ к данным конкретной области и возвращает обычные PHP-массивы, числа либо null. Запросы строятся через Database::table(), а Database::rows() и Database::row() приводят результаты Illuminate Database к принятому в приложении формату массивов.',
        1 => 'Значения передаются через bindings Query Builder. Имена столбцов и направление сортировки нельзя получать из запроса напрямую, поэтому они выбираются только из жёсткого allowlist. Репозиторий может выполнять небольшое правило, непосредственно связанное с хранением, например удалить сектор без связей или деактивировать используемый.',
        2 => 'Не передавайте изменяемый Builder за границы слоя данных без необходимости и не включайте непроверенные значения в raw-выражения. Операцию над несколькими репозиториями объединяет сервис, а граница транзакции задаётся на уровне полного бизнес-сценария.',
      ),
      'examples' => 
      array (
        0 => 
          ...
          'code' => 'public function paginate(int $page, int $perPage, string $sort): array
{
    ...
    array (
      ...
      'paragraphs' => 
      array (
        0 => 'Service Layer используется там, где действие не сводится к одному запросу в одну таблицу. ContactWriteService и ClientWriteService являются единственной прикладной точкой создания и обновления контактов и клиентов: они нормализуют и валидируют основную запись, проверяют дубликаты и сохраняют теги, связи и пользовательские поля. HTML-контроллеры, API-сервисы и процессоры импорта только адаптируют свой ввод и переводят общие WriteException в ответ своего транспорта.',
        1 => 'Сервис может использовать несколько репозиториев и другие сервисы, но не должен зависеть от HTML. Составные операции выполняются через Database::transaction(). Метод открывает транзакцию, если её ещё нет, либо присоединяется к транзакции API-пакета или строки импорта. Поэтому вложенный сервис не пытается открыть отдельную nested transaction.',
...
throw new ApiException(
    ...
    array (
      ...
      'paragraphs' => 
      array (
        0 => 'Например, создание контакта через интерфейс начинается с проверки contacts.create политикой POST-маршрута. ContactController извлекает поля формы и передаёт данные, id тегов, клиентов и пользовательские значения в ContactWriteService. Сервис нормализует и валидирует запись, классифицирует email и одной транзакцией создаёт основную запись со связями; контроллер только переводит WriteException и выполняет redirect.',
        1 => 'Та же операция через API идёт в настроенный для contacts экземпляр ApiController, затем в ContactApiService. API-сервис проверяет структуру JSON, преобразует внешние имена тегов и клиентов в id и вызывает тот же ContactWriteService. Общие предметные ошибки получают стабильный API-код через WriteException, пакетная транзакция охватывает автоматически созданные справочники, а результат журналируется в api_logs.',
        2 => 'Импорт использует ту же границу записи: ContactImportProcessor и ClientImportProcessor преобразуют колонки строки и вызывают ContactWriteService или ClientWriteService внутри транзакции строки. Общая валидация и семантика дубликатов больше не копируются в процессорах; ImportManager переводит WriteException в error или skipped. Для массовой обработки сервису явно отключается только медленная DNS-проверка email.',
      ),
      'examples' => 
      array (
        0 => 
...
public_html/index.php                    # require_once, объект, маршруты',
    ...
    array (
      ...
      array (
        ...
        1 => 'В проекте нет моделей Eloquent и системы миграций: доступ к данным выполняется через Illuminate Database Query Builder, а изменения существующей базы администратор подготавливает и применяет вручную через SQL-клиент. database/schema.sql предназначен только для первой установки и является актуальным снимком полной схемы. Каталога автоматических тестов пока нет, поэтому изменение проверяется PHP lint, сценариями интерфейса и API и просмотром журналов; критичные предметные правила желательно постепенно покрывать модульными или интеграционными тестами.',
