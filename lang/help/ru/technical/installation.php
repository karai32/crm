...
sudo journalctl -u cron --since today --no-pager',
    ...
    array (
      ...
      array (
        ...
        1 => 'Для ИИ-инструмента сервер должен разрешать HTTPS-запросы к generativelanguage.googleapis.com, иметь рабочие корневые сертификаты и расширение cURL. Ключ Gemini хранится только в config/gemini.php. Не добавляйте его в JavaScript, URL, Git или журнал ошибок.',
...
tail -n 100 storage/app.log',
    ...
    array (
      ...
      array (
        ...
        1 => 'Перед обновлением сделайте резервную копию базы и конфигурации. Разверните новый код, выполните composer install по существующему composer.lock, повторно проверьте platform requirements и перезапустите PHP-FPM. Не применяйте database/schema.sql как обновление. Последовательно выполните ещё не применённые SQL-файлы из database/migrations по инструкции версии; автоматического migration runner пока нет.',
