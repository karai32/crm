...
CODE,
        ...
        [
            ...
            'paragraphs' => [
                ...
                'Маршруты /login/verify и /login/resend-code, представление и отправка через MailerService зарегистрированы, и AuthController::login() теперь вызывает TwoFactorService::start() после проверки пароля. completeLogin() выполняется только когда verifyTwoFactor() принимает код, поэтому один верный пароль больше не открывает сессию; постоянный remember-me токен выдаётся в этой же точке, а не сразу после пароля, и его нельзя получить, не пройдя второй фактор.',
                'Одного включения кода было недостаточно: remember-me теперь выдаёт токен только после успешного verifyTwoFactor(), что закрывает брешь, при которой один верный пароль давал постоянный credential. Что осталось: 2FA обязательна для всех учётных записей, а не настраивается по пользователю или роли; смена email не защищена отдельно; события успеха, неудачи и повторной отправки не журналируются; нет recovery и резервных кодов на случай недоступного почтового ящика; лимиты попыток и отправок живут только в сессии конкретного входа, а не централизованно; отказ SMTP во время входа не проверен под реальной нагрузкой. Email-код также защищает слабее TOTP или WebAuthn и зависит от безопасности почтового ящика.',
            ],
            'examples' => [
                [
                    'title' => 'Фактическое поведение',
                    'code' => <<<'CODE'
Current:
...
CODE,
        ...
        [
            ...
            'paragraphs' => [
                ...
                'Auth::ensureAuthenticated() теперь сверяет users.is_active с базой на каждом запросе, требующем сессии, поэтому деактивированная учётная запись теряет доступ на следующем же запросе, а не сохраняет его до конца 30-дневной сессии. Роль по-прежнему хранится только строкой в session user и так же заново не проверяется, поэтому изменённая роль вступает в силу лишь после нового входа или восстановления через remember-me. Смена пароля также не уничтожает другие активные сессии и все remember-токены. При восстановлении через remember-me активность и текущая роль уже проверяются заново.',
...
CODE,
        ...
        [
            ...
            'paragraphs' => [
                'Разрешения уже переведены на fail-closed, политики веб-маршрутов централизованы в Router, а доступ к странице ИИ и её AJAX выровнен до уровня admin. При добавлении permission по-прежнему нужно вручную задать явные значения для существующих пользователей. Живые сессии теперь сверяют активность учётной записи на каждом запросе, поэтому деактивация вступает в силу на следующем запросе, а не сохраняется до конца 30-дневной сессии; оставшийся высокий приоритет — сделать то же самое для смены роли и добавить возможность принудительно отозвать по запросу остальные сессии и remember-токены конкретного пользователя (смена пароля или роли пока применяется только при следующем входе).',
                'Следующий уровень — добавить Secure постоянной cookie, сделать logout POST с CSRF, ввести явные idle и absolute session timeouts, строгую политику паролей и rehash, а также развить 2FA — теперь включённую для каждого входа — до обязательности по роли, recovery-кодов и журналирования событий. LoginThrottle следует перенести в надёжное централизованное хранилище и дополнить IP/risk-ограничениями без возможности простой блокировки чужой учётной записи.',
