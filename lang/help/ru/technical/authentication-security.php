...
CODE,
        ...
        [
            ...
            'paragraphs' => [
                'В схеме есть роли admin и user. Администратор получает безусловный доступ в Auth::can() и только он управляет пользователями, API-ключами и API-журналами через requireAdmin(). Для обычного пользователя индивидуальные решения хранятся в user_permissions по составному ключу user_id + permission_key.',
                ...
                'Меню скрывает недоступные пункты для удобства, но не является защитой. Каждый контроллер обязан повторно вызвать requireLogin(), requirePermission() или requireAdmin() до чтения данных и особенно до изменения состояния. Проверять право только в представлении или JavaScript нельзя: URL и POST можно вызвать напрямую.',
            ],
            'examples' => [
                [
                    'title' => 'Выбор серверной проверки',
                    'code' => <<<'PHP'
// Любой вошедший пользователь
Auth::requireLogin();

// Обычный пользователь с конкретной возможностью или admin
...
PHP,
        ...
        [
            ...
            'paragraphs' => [
                'Auth::can() сначала требует существование session user, затем без запросов разрешает всё роли admin. Специальное имя users.manage всегда запрещено неадминистратору, хотя в permissionDefinitions его нет. Для остальных ключей userPermissions() один раз за HTTP-запрос читает строки пользователя и кэширует ассоциативный массив в static-переменной.',
                'Критическая особенность текущего контракта: отсутствующая строка означает разрешение, а не запрет. Формула имеет вид «если ключ записан — использовать is_allowed, иначе true». При создании и редактировании обычного пользователя UserController обычно сохраняет явные 1 или 0 для всех известных ключей, но новый permission key автоматически станет доступен старым пользователям до пересохранения их настроек.',
                'Если запрос user_permissions завершится исключением, метод возвращает пустой массив, после чего can() также разрешит все обычные ключи. Это fail-open поведение. Для безопасной модели отсутствие данных и ошибка базы должны запрещать операцию; новые разрешения должны получать явный default deny либо миграцию значений. Такое изменение потребует проверить существующих пользователей, иначе после исправления они неожиданно потеряют действия.',
            ],
            'examples' => [
                [
...
no session                         → false
role = admin                       → true
permission = users.manage, user    → false
explicit is_allowed = 1            → true
explicit is_allowed = 0            → false
permission row is absent           → true   ← fail-open
permission query failed            → true   ← fail-open for normal keys
CODE,
                ],
            ],
        ],
        [
            ...
            'paragraphs' => [
                'Router только сопоставляет метод и путь; централизованного middleware-реестра разрешений нет. Поэтому проверка находится в начале каждого action. Чтение contacts и clients использует requireLogin, изменения — отдельные create/edit/delete permissions. Импорт, экспорт, сектора, теги и пользовательские поля защищают все свои actions соответствующим разрешением. Управление пользователями и API-ключами требует администратора.',
                'AJAX-контроллер возвращает JSON 401 или 403 через guard(), а HTML-контроллеры перенаправляют неаутентифицированного пользователя на /login и печатают 403 Access denied при недостатке права. При добавлении маршрута разработчик обязан отдельно зарегистрировать его в public_html/index.php и поставить guard в action; забытый вызов автоматически защищён не будет.',
                'Есть несогласованность в ИИ-сценарии. Страница /ai вручную разрешена только администратору, но geminiCompany(), saveContactCompany() и skipContactCompany() вызывают guard() без permission и доступны любому вошедшему пользователю при прямом POST. inspectEmailBatch() также требует только login. CSRF не заменяет авторизацию: он подтверждает происхождение запроса из сессии, но не право пользователя на действие.',
            ],
            'examples' => [
                [
...
{
    if (!$this->guard('contacts.edit')) {
        return;
    }

    // validate identifiers and values, then mutate
...
CODE,
        ...
        [
            ...
            'paragraphs' => [
                'Репозитории в основном используют подготовленные PDO-запросы. Там, где динамически выбирается сортировка или экспортируемая колонка, значение должно проходить фиксированный белый список до включения в SQL. Приведение id к int полезно как нормализация, но не заменяет проверку существования сущности и разрешения на неё.',
...
CODE,
        ...
        [
            ...
            'paragraphs' => [
                'Наивысший приоритет — перевести разрешения на fail-closed: неизвестный ключ и ошибка чтения должны давать false, а добавление permission сопровождаться миграцией. Одновременно нужно выровнять guards ИИ-AJAX с административной страницей, проверять активность и текущую роль пользователя в живой сессии и реализовать отзыв всех сессий и remember-токенов при деактивации, смене пароля или роли.',
            ...
            'examples' => [
                [
                    ...
                    'code' => <<<'CODE'
P0  fail-closed permissions
P0  correct AI AJAX authorization
