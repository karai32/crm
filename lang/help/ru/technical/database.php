...
array (
  ...
  array (
    ...
    2 => 'Приложение не использует ORM. Новые и постепенно обновляемые репозитории строят запросы через Illuminate Database Query Builder, а существующий SQL через PDO продолжает работать во время поэтапного перехода. Оба способа возвращают ассоциативные массивы и используют одно подключение. PDO настроен с ATTR_STRINGIFY_FETCHES, поэтому числовые значения из SELECT могут приходить как строки; код явно приводит идентификаторы, счётчики и флаги к int там, где это важно.',
  ),
  'examples' =>
  array (
    0 =>
