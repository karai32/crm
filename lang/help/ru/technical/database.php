...
array (
  ...
  array (
    ...
    2 => 'Приложение не использует модели Eloquent: репозитории, авторизация, отчёты и экспорт единообразно строят запросы через Illuminate Database Query Builder и возвращают ассоциативные массивы. Raw-выражения применяются только для возможностей MySQL вроде GROUP_CONCAT, CASE и специальных функций. Драйвер настроен с ATTR_STRINGIFY_FETCHES, поэтому числовые значения из SELECT могут приходить как строки; код явно приводит идентификаторы, счётчики и флаги к int там, где это важно.',
  ),
  'examples' =>
  array (
    0 =>
...
array (
  ...
  array (
    ...
    1 => 'Все запросы используют одно подключение Illuminate Database. Составные операции следует выполнять через Database::transaction(): helper открывает и завершает транзакцию, если является её владельцем, либо присоединяет callback к уже открытой транзакции API-пакета или строки импорта. Исключение автоматически приводит к rollback владельца и пробрасывается дальше.',
...
array (
  ...
  'title' => 'Изменение схемы',
  'paragraphs' =>
  array (
    0 => 'database/schema.sql — полный снимок для чистой установки. В начале он отключает проверку внешних ключей и выполняет DROP TABLE, поэтому запуск этого файла на рабочей базе уничтожит данные. В проекте нет системы миграций, последовательных SQL-файлов обновления и таблицы учёта версий. Изменения существующей базы администратор выполняет вручную через SQL-клиент.',
    1 => 'Каждое изменение схемы сначала подготавливается и проверяется на копии базы, затем вручную применяется к рабочей базе и фиксируется во внешнем журнале развёртывания. После проверки database/schema.sql обновляется, чтобы новые установки сразу получали актуальную структуру. Текущий снимок уже содержит fail-closed значение разрешений, UNIQUE-индексы и CHECK-ограничения.',
  ...
  array (
    0 =>
    array (
      'title' => 'Источник актуальной схемы',
      'code' => 'database/
└── schema.sql    # полная схема для чистой установки',
    ),
    1 =>
    array (
      'title' => 'Пример ручного изменения схемы',
...
WHERE TABLE_SCHEMA = DATABASE()
  AND TABLE_NAME = \'contacts\';',
    ...
    array (
      ...
      'code' => '[ ] ручное изменение проверено на копии базы
[ ] рабочая база обновлена и изменение записано во внешний журнал
[ ] database/schema.sql обновлён для чистой установки
