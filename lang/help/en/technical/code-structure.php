...
CODE,
        ...
        ['id' => 'code-conventions', 'title' => 'Conventions and current limitations', 'paragraphs' => [
            ...
            'The project has no Eloquent models or migration system: data access uses Illuminate Database Query Builder, and the administrator prepares and applies changes to an existing database manually through an SQL client. database/schema.sql is only for initial installation and represents the current complete schema. There is no automated test directory yet, so changes are checked with PHP lint, interface/API scenarios, and logs; critical domain rules should gradually receive unit or integration tests.',
